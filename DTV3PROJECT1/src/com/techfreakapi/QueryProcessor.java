...
public class QueryProcessor {
	...
	public Map<Integer, ArrayList<String>> processQuery(QueryParameter queryParameter, CSVFileReader csvFileReader) {
		...
		if ((selectedColumnNames != null) && (tableName != null)) {
			// checking presence of table name and columns
			if ((groupByColumn == null) && (whereClause == null) && (orderByColumn == null)) {
				// Go inside Only if group by and Order By and Where Clause is Not Present
				if (selectedColumnNames[0].trim().equals("*")) { // check for Presence of *
					dataSet = csvFileReader.readData(headers, headers);
				} else { // Call if * is Not there in query
					dataSet = csvFileReader.readData(selectedColumnNames, headers);
				} // Go Inside if Where Clause is there
			} else if ((whereClause != null) && (groupByColumn == null) && (orderByColumn == null)) {
				if (selectedColumnNames[0].trim().equals("*")) {
					dataSet = csvFileReader.readData(headers, headers, whereClause, logicalConditions);
				} else {
					dataSet = csvFileReader.readData(selectedColumnNames, headers, whereClause, logicalConditions);
				}
				// move in for only group by
			} else if (groupByColumn != null && whereClause == null && orderByColumn == null) {
				if (selectedColumnNames[0].trim().equals("*")) { // check for Presence of *
					
				} else { // Call if * is Not there in query
					// select ColumName with Group by
				}
				// move in for where with Order by
			} else if (orderByColumn != null && groupByColumn == null) {
				if (selectedColumnNames[0].trim().equals("*")) { // check for Presence of *
					// select * with order by
					dataSet = csvFileReader.readData(headers, headers, orderByIndexInMap,whereClause,logicalConditions);
				} else { // Call if * is Not there in query
					dataSet = csvFileReader.readData(selectedColumnNames, headers, orderByIndexInMap,whereClause,logicalConditions);
				}
				// move in where with group by
			} else if (whereClause != null && groupByColumn != null && orderByColumn == null) {

				if (selectedColumnNames[0].trim().equals("*")) { // check for Presence of *
					// select * from where with group by
				} else { // Call if * is Not there in query
					// select ColumName from where with Group by
				}
				// move in where with orderby
			} else if (whereClause != null && orderByColumn != null && groupByColumn == null) {

				if (selectedColumnNames[0].trim().equals("*")) { // check for Presence of *
					// select * from where with orderBy
				} else { // Call if * is Not there in query
					// select ColumName from where with OrderBy
				}
				// move in for where with group by and order by
			} else if (whereClause != null && orderByColumn != null && groupByColumn == null) {

				if (selectedColumnNames[0].trim().equals("*")) { // check for Presence of *
					// select * from where with group by and orderBy
				} else { // Call if * is Not there in query
					// select ColumName from where with group by and OrderBy
				}
			}
		} else {
			return null;
